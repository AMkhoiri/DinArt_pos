...
<div class="mr-2 ml-2">
 <div class="card border border-bottom-0 border-left-0 border-right-0  border-success">
      <div class="card-header text-success">
        <div class="row">
          <div class="col-md-6"><h3 class="card-title font-weight-bold"><a class="fas fa-arrow-left text-success mr-3" href="{{ route('cs-transaksi') }}">  </a>Invoice Transaksi</h3></div>
    ...
    <div class="card-body">
        ...
        <div class="card-footer  ">
          <div class="row">
            <div class="col-md-6">
               @if ($order->status_produksi==null)
              <a href="{{ route('cs-transaksi-skip-produksi',$order->id) }}" class="btn btn-sm btn-success pl-4 pr-4 mb-2" data-toggle="tooltip" data-placement="right" title="Tidak ada item yang perlu dikirim ke produksi">Skip <span class="fas fa-angle-right pl-2 " ></span><span class="fas fa-angle-right " ></span><span class="fas fa-angle-right " ></span> </a>
              @endif
            </div>
            <div class="col-md-6 text-right">
               @if ($order->status_produksi==null)
                <a href="{{ route('cs-transaksi-submit-produksi',$order->id) }}" class="btn btn-sm btn-success">Send Item Selected <span class="fas fa-check" ></span></a>
                <a href="{{ route('cs-transaksi-submit-all-produksi',$order->id) }}" class="btn btn-sm btn-outline-success pl-2 pr-2">Send All Item <span class="fas fa-paper-plane " ></span> </a> 
               @endif
            </div>
          </div>
...
<script>
   ...
   function checkItem(id) 
   {
      csrf_token= $('meta[name="csrf-token"]').attr('content') ;
         ...
         $.ajax({
            url:"{{ url('cs/transaksi/form-produksi') }}"+'/'+id+'/check',
            type:"POST",
            data: {
                '_method':'POST',
                '_token':csrf_token 
