...
{{-- /modal detail transaksi --}}
 ...
 <div class="card">
      ...
      <div class="card-body pt-1">
        <a href="{{ route('cs-newtransaksi') }}" class="btn  pr-4 pl-4 bg-gradient-success float-right " style="font-size: 13px;" ><span class="fa fa-cart-plus" style="font-size: 14px;" > </span> Transaksi Baru</a>
            ...
            <td class=""> 
              @if ($o->status_order!=='selesai')
                ...
                @if ( $o->status_pembayaran !== null && $o->status_produksi == null)
                    <a href="{{ route('cs-transaksi-form-produksi',$id_o_hash) }}" class="btn btn-sm btn-info " data-toggle="tooltip" data-placement="top" title="Kirim ke produksi"  {{-- style="color: white; background-color: #008080" --}} ><span class="fas fa-print" ></span></a>
                @elseif($o->status_pembayaran !== null && $o->status_produksi !== null)
                    <a href="{{ route('cs-transaksi-form-produksi',$id_o_hash) }}" class="btn btn-sm btn-info " data-toggle="tooltip" data-placement="top" title="Lihat Detail produksi"  {{-- style="color: white; background-color: #008080" --}} ><span class="fas fa-print" ></span></a>    
                @else
                ...
                @else
                    <a href="{{ route('cs-transaksi-bayar',$id_o_hash) }}" class="btn btn-sm btn-warning " data-toggle="tooltip" data-placement="top" title="Lanjutkan pembayaran"><span class="fas fa-money-bill-alt" ></span></a>
                @endif
...
<script>
  ...
  function detailModal(id) 
  {
    ...
    // $('#detailModal')[0].reset();
    $.ajax({
      url: "{{ url('cs/transaksi')}}"+'/'+id+"/detail",
      type: "GET",
      dataType: "JSON",
...
$('body').on('click', '.btn-done', function(event) {
  ...
  Swal.fire({
      ...
      if (result.value) {  
        // window.location.href = "{{ url('cs/transaksi') }}"+'/'+id+'/done'

        $.ajax({
            url:"{{ url('cs/transaksi') }}"+'/'+id+'/done',
           type: "GET",
           dataType: "JSON",
        ...
        success: function(data) {
          ...
          if (result.value) {
            ...
            Swal.fire({
                ...
                // window.location.href = "{{ url('cs/transaksi') }}"+'/'+id+'/cetak'
                if (result.value) {
                      window.open(
                              "{{ url('cs/transaksi') }}"+'/'+id+'/cetak',
                              '_blank' 
                              );
  ...
  event.preventDefault();
    ...
    Swal.fire({
          ...
          if (result.isConfirmed) {
             ...
             window.open(
                        "{{ url('cs/transaksi') }}"+'/'+idd+'/cetak/thermal',
                        '_blank' 
                        );
...
$('body').on('click', '.btn-cancel', function(event) {
  ...
  Swal.fire({
      ...
      if (result.value) {  
        ...
        window.location.href = "{{ url('cs/transaksi') }}"+'/'+id+'/cancel'
