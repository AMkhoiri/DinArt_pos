...
<div class="row">
  <div class="col-md-5">
    <div class="card border border-bottom-0 border-left-0 border-right-0  border-success">
      ...
      {{-- <form  id="submit-transaksi"> --}}
        <form  method="post" action="{{ route('admin-continue-payment',$idorderr_hash) }} "  enctype="multipart/form-data">  
      @csrf 
  ...
  <div class="col-md-5">
   ...
   <div class="col-md-2 " >
     ...
     <a href="{{ route('admin-transaksi') }}" class="btn btn-outline-danger mb-3 font-weight-bold" style="font-size: 14px;"><span class="fa fa-arrow-left" ></span> Cancel Transaction</a>
   </div>
</form>
</div>
...
$("#form_add_list").submit(function(event) {
  ...
  $.ajax({
    url:"{{ url('admin/transaksi')}}"+'/'+id+'/additem',
    type:"POST",
    data:{
      id_barang:barang,
      panjang:panjang,
    ...
    success:function(response) 
    {
     ...
     // console.log(response);
      if(response)
      {
        ...
        {
          console.log(response.catatan_item);
...
function showCatatan(id) 
{
  ...
  $.ajax({
        ...
        url: "{{ url('admin/transaksi/item')}}"+'/'+id+"/catatan",
        type: "GET",
        dataType: "JSON",
...
<script>
   ...
   function deleteItem(id) 
   {
      csrf_token= $('meta[name="csrf-token"]').attr('content') ;
         ...
         $.ajax({
            url:"{{ url('admin/transaksi/item') }}"+'/'+id+'/delete',
            type:"POST",
            data: {
                '_method':'DELETE',
                '_token':csrf_token 
